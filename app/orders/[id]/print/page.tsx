...
export default async function PrintOrderPage(props: Props) {
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-4 md:p-8 font-sans print:bg-white print:p-0">
            ...
            <div id="invoice-content" className="max-w-[210mm] mx-auto bg-white shadow-2xl print:shadow-none print:w-full print:max-w-none px-6 pt-4 md:px-10 relative">
                
                {/* Dynamic Header */}
                {settings?.header && (
                    <div className="absolute top-0 left-0 right-0 h-[4.5cm] p-4 bg-white" dangerouslySetInnerHTML={{ __html: settings.header }}></div>
                )}

                <div style={{ paddingTop: settings?.header ? '4.5cm' : '0' }}>
                ...
                {/* Dynamic Footer */}
                {settings?.footer && (
                    <div className="mt-8 pt-4 border-t-2 border-dashed" dangerouslySetInnerHTML={{ __html: settings.footer }}></div>
                )}
            </div>
        </div>
    );
